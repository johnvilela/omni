...
- **Persistence: SQLite `pins` table** (`chat_id PK, message_id, mode`) in the CREATE list (not the ALTER migration list — that's column-adds only). Restart re-attaches via the ConnectTelegram hook (this page originally claimed "queues are empty then, so the count self-corrects to 0" — that predates queue persistence; see the v0.11.0 note below: queued-but-unstarted messages now replay via `replayQueue()` on boot instead of dying).
...
**Fixed in v0.11.0:** the earlier limit here — "full-mode list = `RecentSessions(5)`; an older unread session can drop off the list" — is gone. `RecentSessions` now orders `(unread != '') DESC, id DESC` instead of newest-first, so a session holding an unread answer can't drop off the cap; this fixes `/pin full` and the `/sessions` keyboard in the same query change. See [[decisions/background-session-queue]].
