...
- **`install.sh`** — Bash one-shot release downloader. It resolves `OMNI_VERSION` (or latest), downloads the matching Omni binaries and `checksums.txt`, verifies checksums, and installs the binaries. It writes the server, guardian, and local ai-memory user units; retries remain an in-place Omni upgrade. The ai-memory service uses FTS without local embeddings, disables scheduled improvement and backfill, and caps memory at 512 MiB for the 4 GiB/HDD target. Raw-observation pruning defaults to 30 days, configurable with `/memory_retention`; only consolidated sessions are eligible. `OMNI_SKIP_DEPS=1` skips browser dependencies, not memory. The existing agent workspace and browser profile are preserved. Headless session-bus and lingering guards remain in place.
