...
func TestApplyTools(t *testing.T) {
	...
	if got := srv.applyTools("just prose"); got != "just prose" {
		t.Fatalf("passthrough = %q", got)
	}

	// add
	got := srv.applyTools(`Sure!
TOOL:cron_add {"schedule":"0 8 * * *","kind":"message","text":"drink water"}`)
	...
	got = srv.applyTools(`TOOL:cron_add {"schedule":"99 8 * * *","kind":"message","text":"x"}`)
	if !strings.Contains(got, "⚠") {
		t.Fatalf("invalid schedule reply = %q", got)
	}
	// invalid kind
	got = srv.applyTools(`TOOL:cron_add {"schedule":"0 8 * * *","kind":"nuke","text":"x"}`)
	if !strings.Contains(got, "⚠") {
	...
	// edit
	got = srv.applyTools(`TOOL:cron_edit {"id":1,"schedule":"30 8 * * *","kind":"message","text":"hydrate"}`)
	...
	if got = srv.applyTools(`TOOL:cron_edit {"id":99,"schedule":"0 8 * * *","kind":"message","text":"x"}`); !strings.Contains(got, "⚠") {
		t.Fatalf("edit unknown = %q", got)
	}

	// delete
	if got = srv.applyTools(`TOOL:cron_delete {"id":1}`); !strings.Contains(got, "#1") {
		t.Fatalf("delete reply = %q", got)
	}
	if cs, _ := store.Crons(); len(cs) != 0 {
		t.Fatalf("crons after delete = %+v", cs)
	}
	if got = srv.applyTools(`TOOL:cron_delete {"id":1}`); !strings.Contains(got, "⚠") {
